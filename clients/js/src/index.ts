...
interface QueuedLog extends Omit<LogEntry, 'service'> {
  level: LogLevel;
  message: string;
  timestamp: string;
  correlationId?: string;
...
class LogCollectorClient {
  private config: Required<Omit<LogCollectorConfig, 'correlationIdFn'>> & {
    correlationIdFn?: () => string | undefined;
  };
  ...
  private setupConsoleCapture(): void {
    const levels: Array<{ method: keyof Console; level: LogLevel }> = [
  ...
  async flush(): Promise<void> {
    ...
    const logs: LogEntry[] = batch.map((entry) => ({
      ...entry,
  ...
  private handleSendError(error: unknown, batch: QueuedLog[]): void {
    if (this.isCollectorAvailable) {
      console.warn(
        `[LogCollector] Collector unavailable: ${error instanceof Error ? error.message : error}. Logs will be re-queued.`
      );
...
// Singleton instance
let instance: LogCollectorClient | null = null;
...
export function getLogCollector(): LogCollectorClient | null {
  return instance;
}
